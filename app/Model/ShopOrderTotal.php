...
class ShopOrderTotal extends Model
{
    ...
    public static function getShippingMethod()
    {
        ...
        if ($shippingMethod) {
            ...
            $shipping_method = (new \App\Model\ShippingPrice)->where('code', $shippingMethod)->first();
            if($shipping_method)
                $arrShipping = [
                    'title' => $shipping_method->name,
                    'code' => 'shipping',
                    'value' => sc_currency_value($shipping_method->price),
                    'text' => sc_currency_render_symbol($shipping_method->price),
    ...
    /**
     * Get payment method fee value
     */
    public static function getPaymentFee()
    {
        $value = 0;
        $text = 0;
        ...
        if ($payment_code) {
            $payment_method = \App\Model\ShopPaymentMethodItem::where('code', $payment_code)->first();
            $value = $payment_method->fee ?? 0;
            $text = sc_currency_render_symbol($payment_method->fee ?? 0);
        }
        $totalMethod = [
            'title' => 'Payment Fee',
            'code' => 'payment_fee',
