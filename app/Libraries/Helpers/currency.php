...
if (!function_exists('sc_currency_render_symbol') && !in_array('sc_currency_render_symbol', config('helper_except', []))) {
     ...
     * @return  [type]                        [return description]
     */
    function sc_currency_render_symbol(float $money, $currency = null, $space_between_symbol = true, $include_symbol = true)
    {
...
if (!function_exists('sc_currency_format') && !in_array('sc_currency_format', config('helper_except', []))) {
    function sc_currency_format(float $money, $precision = 0)
    {
        return ShopCurrency::format($money, $precision);
    }
}
