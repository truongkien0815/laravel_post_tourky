...
class CartController extends Controller
{
    ...
    public function checkout()
    {
        ...
        if(!session()->has('dataCheckout'))
            session(['dataCheckout' => $cart_content]);
    ...
    public function checkoutConfirm()
    {
        ...
        if(!empty($data['payment_method']))
            session()->put('paymentMethod', $data['payment_method']);
        if(!empty($data['bank_code']))
            session()->put('bank_code', $data['bank_code']);

        $shippingAddress = session('shippingAddress')??'';

        //shipping
        if(!empty($data['name']))
            $shippingAddress['fullname'] = $data['name'];
        if(!empty($data['phone']))
            $shippingAddress['phone'] = $data['phone'];
        if(!empty($data['email']))
            $shippingAddress['email'] = $data['email'];
        if(!empty($data['ward']))
            $shippingAddress['ward'] = $data['ward'];
        if(!empty($data['district']))
            $shippingAddress['district'] = $data['district'];
        if(!empty($data['province']))
            $shippingAddress['province'] = $data['province'];
        if(!empty($data['delivery']))
            $shippingAddress['delivery'] = $data['delivery'];
        if(!empty($data['address_line1']))
            $shippingAddress['address_line1'] = $data['address_line1'];

        $customer_ward = \App\Model\LocationWard::where('name', request('ward'))->first();
        if($customer_ward)
        {
            $address_full = array_filter([ request('address_line1'), $customer_ward->type_name, request('district'),request('province') ]);
            $address_full = implode(', ', $address_full);
            $shippingAddress['address_full'] = $address_full;
        }

        //pick up
        if(!empty($data['pick_up-name']))
            $shippingAddress['fullname'] = $data['pick_up-name'];
        if(!empty($data['pick_up-phone']))
            $shippingAddress['phone'] = $data['pick_up-phone'];
        if(!empty($data['pick_up-email']))
            $shippingAddress['email'] = $data['pick_up-email'];
        ...
        if($data){
            if($data['delivery'] == 'shipping'){
                $data['name'] = $shippingAddress['fullname'];
        ...
        return view($this->templatePath .'.cart.checkout-confirm', $this->data);
    }
    ...
    public function processCheckout(Request $request)
    {
        ...
        session()->forget('bank_code');
        if(!empty($data['bank_code']))
            session()->put('bank_code', $data['bank_code']);
        // dd($data);
        $paymentGetway = sc_payment_getway($paymentMethod);

        if ($paymentGetway) {
            // Check payment method
            return (new $paymentGetway)->processOrder();
        }
        else
    ...
    public function completeOrder()
    {
        ...
        // dd('done');
            ...
            if ($checkContent || $checkContentCustomer) 
            {
                ...
                $orderDetail = '';
                $orderDetail .= '<table border="0" cellpadding="5" cellspacing="0" style="width: 100%; margin: auto;">
                                <tr style="background: #f1f1f1; font-weight: 700;">
                                    <td width="60">' . sc_language_render('email.order.sort') . '</td>
                                    <td>' . sc_language_render('email.order.sku') . '</td>
                                    <td>' . sc_language_render('email.order.name') . '</td>
                                    <td width="70" align="center">' . sc_language_render('email.order.price') . '</td>
                                    <td width="70" align="center">' . sc_language_render('email.order.qty') . '</td>
                                    <td width="70" align="right">' . sc_language_render('email.order.total') . '</td>
                                </tr>';
                foreach ($data['details'] as $key => $detail) 
                {
                    ...
                    $orderDetail .= '<tr>
                                        ...
                                        <td>' . $detail['sku'] . '</td>
                                        <td>' . $nameProduct . '</td>
                                        <td align="center">' . sc_currency_render($detail['price'], '', '', '', false) . '</td>
                ...
                $config = [
                    ...
                    'subject' => $checkContent->subject??sc_language_render('email.order.email_subject_to_admin', ['order_id' => $orderID]),
                ];
                // sc_send_mail('email.content', $dataView, $config, []);
                ...
                $contentCustomer = htmlspecialchars_decode($checkContentCustomer->text);
                ...
                $config = [
                    ...
                    'subject' => $checkContentCustomer->subject ?? sc_language_render('email.order.email_subject_customer', ['order_id' => $orderID]),
                ];
                ...
                // sc_send_mail('email.content', $dataView, $config, $attach);
    ...
    private function _orderSuccess()
    {
        ...
        if(view()->exists($this->templatePath . '.cart.shop_order_success'))
        {
            // dd(session('orderIDs'));
            $orderInfos = ShopOrder::with('details')->where('id', session('orderIDs'))->get()->toArray();
            // dd($orderInfos);
            return view(
                $this->templatePath . '.cart.shop_order_success',
                [
