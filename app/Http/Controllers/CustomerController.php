...
class CustomerController extends Controller
{
        ...
        if($check_user!='')
        {
            if($check_user->status==0)
                return response()->json(
                    [
                        'error' => 1,
                        'msg'   => __('Tài khoản chưa được kích hoạt')
                    ]
                );
            ...
            if (Auth::attempt($login, $remember_me)) {
                ...
                return response()->json(
                    [
                        'error' => 1,
                        'msg'   => __('Email hoặc mật khẩu không đúng')
                    ]
                );
            }
        } 
            ...
            return response()->json(
                [
                    'error' => 1,
                    'msg'   => __('Tài khoản khôgn tồn tại!')
                ]
            );
        }
    }
    ...
    public function createCustomer(Request $request){
        ...
        if($new_cus->id)
        {
            ...
            $name_admin_email = setting_option('company_name');
            $url_web = url('/');
