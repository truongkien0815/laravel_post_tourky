...
class AppConfig extends ConfigDefault
{
    ...
    public function getData()
    {
        ...
        if (!empty($discount) && !$check['error']) {
            ...
            if ($check['content']['type'] == 'percent') {
                $value = $subtotalWithTax * $check['content']['reward'] / 100;
            } else {
                $value = sc_currency_value($check['content']['reward']);
            }
            // dd($subtotalWithTax * $check['content']['reward'] / 100);
            $dataStore = [];

            $arrData = array(
                'title'      => $this->title . ':<b> ' . $discount . '</b>',
                'code'       => $this->configCode,
                'key'        => $this->configKey,
                'image'      => $this->image,
                'permission' => self::ALLOW,
    ...
    public function endApp($data = []) {
        (new FrontController)->apply($data['code']);
        return;
    }
}
