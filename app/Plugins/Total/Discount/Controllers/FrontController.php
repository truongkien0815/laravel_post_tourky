...
class FrontController extends Controller
{
    ...
    public function useDiscount()
    {
        ...
        if ($check['error'] == 1) {
            ...
            if ($content['type'] === 1) {
                ...
                if (request()->route()->getPrefix() == '/cart') {
                    $view = $this->templatePath . '.cart.render_total';
                } else {
                    $view = $this->templatePath . '.cart.render_total';
                }
                if (view()->exists($view)) {
    ...
    public function removeDiscount()
    {
        ...
        $objects = ShopOrderTotal::getObjectOrderTotal();
        $dataTotal = ShopOrderTotal::processDataTotal($objects);
        if (view()->exists($this->templatePath . '.cart.render_total')) {
            $html = view($this->templatePath . '.cart.render_total')->with(['dataTotal' => $dataTotal])->render();
        }
