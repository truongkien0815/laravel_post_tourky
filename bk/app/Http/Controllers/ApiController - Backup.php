...
class ApiController extends Controller
{
    ...
    {
        $search = $request->name;
        $dataName = Theme::join('join_category_theme', 'join_category_theme.id_theme', '=', 'theme.id')
            ->join('category_theme', 'category_theme.categoryID', '=', 'join_category_theme.id_category_theme')
            ->where('title', 'LIKE', "%$search%")
            ->select('theme.title','theme.slug', 'category_theme.categorySlug')
            ->get();
        $result = "";
        if($dataName)
        foreach ($dataName as $item) {
            $result .= '<li>
                    <div class="title-item-search">
                        <a href="'.route('tintuc.details',array($item->categorySlug,$item->slug)).'">'.$item->title.'</a>
                    </div>
                </li>';
        }
        return $result;
    ...
    public function LoadVueWithPagination(Request $request)
    {
        $search = $request->name;
        $dataName = Theme::join('join_category_theme', 'join_category_theme.id_theme', '=', 'theme.id')
            ->join('category_theme', 'category_theme.categoryID', '=', 'join_category_theme.id_category_theme')
            ->select('theme.title','theme.slug', 'category_theme.categorySlug', 'theme.thubnail', 'theme.price_promotion', 'theme.price_origin')
            ->paginate(4);
        $response = [
            'pagination' => [
                'total' => $dataName->total(),
                'per_page' => $dataName->perPage(),
                'current_page' => $dataName->currentPage(),
                'last_page' => $dataName->lastPage(),
                'from' => $dataName->firstItem(),
                'to' => $dataName->lastItem()
            ],
            'data' => $dataName
        ];
        ...
        // $result = "";
        // if($dataName):
        //     $url_img = 'images/products';
        //     foreach ($dataName as $item) {
        //         $result .= '<div>
        //                 <div class="thumb-item">
        //                     <a href="'.route('tintuc.details',array($item->categorySlug,$item->slug)).'"><img src="'.asset('images/product/'.$item->thubnail).'"></a>
        //                 </div>
        ...
        if($rows):
            ...
            foreach ($rows as $row){
                ...
                if($price_promotion == ''):
                    ...
                    $check_product = Helpers::check_product_variable($row->id);
                    if($check_product){
                        $btn_mua_ngay = '<a rel="nofollow" id="tbl_add_cart_'.$row->id.'" class="btn_click_postype btn btn-cart-list" href="'.route('tintuc.details',array($row->categorySlug,$row->slug)).'"><span class="fa fa-shopping-cart"></span> Mua Ngay</a>';
                    } else{
                        $btn_mua_ngay = '<a rel="nofollow" id="tbl_add_cart_'.$row->id.'" class="btn_click_postype btn btn-cart-list" data-id="'.$row->id.'" data-name="'.$row->title.'" data-summary="'.$row->theme_code.'" data-price="'.$price_pastion.'" data-quantity="1" data-image="'.url('/images/product/')."/".$row->thubnail.'" tabindex="-1"><span class="fa fa-shopping-cart"></span> Mua Ngay</a>';
                    }
                    $result .= '
