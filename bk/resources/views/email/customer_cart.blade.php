...
<p>E-mail: {{$email }}</p>
<p>Tỉnh/Thành phố: {{$cart_province }}</p>
<p>Quận/Huyện: {{$cart_district }}</p>
<p>Phường/Xã: {{$cart_ward }}</p>
<p>Địa chỉ: {{$address }}</p>
...
?>
<p>Phí giao hàng: <b style="color: #F00;">+{{$shipping_fee}}</b> {!!Helpers::get_option_minhnn('currency')!!}</p>
...
<tr>
    <td colspan="4" align="center" valign="middle" style="border: 1px solid #eaeaea; padding:5px 10px;">
        ...
        @endif
        <p><b>Phí giao hàng:</b> <b style="color: #F00;">+{{$shipping_fee}} {!!Helpers::get_option_minhnn('currency')!!}</b></p>
        <p><b>Tổng tiền:</b> <b style="color: #F00;">{!!WebService::formatMoney12($total)!!}  {!!Helpers::get_option_minhnn('currency')!!}</b></p>
    </td>
</tr>
</table>
<p>--</p>
