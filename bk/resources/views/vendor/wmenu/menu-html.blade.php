...
<link href="{{asset('vendor/harimayco-menu/style.css?ver=1')}}" rel="stylesheet">
											...
											<ul class="outer-border">
												...
												<li class="control-section accordion-section add-page" id="add-page">
													...
													<div class="accordion-section-content ">
														<div class="inside">
															<div class="customlinkdiv" id="customlinkdiv">
																@include('vendor.wmenu.includes.category_items')
												...
												<li class="control-section accordion-section add-page" id="add-page">
													...
													<div class="accordion-section-content ">
														<div class="inside">
															<div class="customlinkdiv" id="customlinkdiv">
																@include('vendor.wmenu.includes.page_items')
																...
																<div class="menu-item-settings" id="menu-item-settings-{{$m->id}}">
																	...
																	<div class="form-group form-group-image-box ">
																		<div class="image-box-container" style="width: 44px;">
																		    <div class="custom-image-box image-box">
																		        <input type="hidden" value="{{$m->icon}}" id="icon-{{$m->id}}" class="image-data">

																		        @if($m->icon!='')
																		        <img src="{{asset($m->icon)}}" class="icon-{{$m->id}} preview_image">
																		        @else
																		        <img src="{{ asset('assets/images/placeholder.png') }}" class="icon-{{$m->id}} preview_image">
																		        @endif

																		        <div class="image-box-actions">
																		            <a class="ckfinder-popup" id="image-{{ $m->id ?? 0 }}" data="icon-{{$m->id}}" data-show="icon-{{$m->id}}">Choose image</a>
																		        </div>
																		    </div>
																		</div>
																		@if($m->icon!='')
																			<button type="button" class="btn btn-danger remove-icon" data-img="{{ asset('assets/images/placeholder.png') }}"><i class="fa fa-trash-o"></i></button>
																		@endif
																	</div>

																	<div class="form-group">
																		<label>Mô tả</label>
																		<textarea class="menu-content w-100" rows="2">{!! $m->content !!}</textarea>
																	</div>
																	<div class="form-group">
																		<label for="edit-menu-item-classes-{{$m->id}}"> Class CSS (optional)
																			<br>
																			<input type="text" id="clases_menu_{{$m->id}}" class="widefat code edit-menu-item-classes  form-control" name="clases_menu_{{$m->id}}" value="{{$m->class}}">
																		</label>
																	</div>
																	...
																	<div class="form-group">
																		<label for="edit-menu-item-url-{{$m->id}}"> Url
																			<br>
																			<input type="text" id="url_menu_{{$m->id}}" class="widefat code edit-menu-item-url  form-control" id="url_menu_{{$m->id}}" value="{{$m->link}}">
																		</label>
																	</div>
